...
public class AuthController {
    ...
    //User submits a login request (loginRequest) with a username and password.
    //
    //UsernamePasswordAuthenticationToken object is created using these credentials.
    //
    //authenticationManager.authenticate() processes the authentication request:
    //
    //If credentials are valid → returns an authenticated Authentication object.
    //
...
//      set the authentication
        ...
        UserDetailsImpl userDetails = (UserDetailsImpl) authentication.getPrincipal(); //authentication.getPrincipal() retrieves the authenticated user's details from the authentication object.

        String jwtToken = jwtUtils.generateTokenFromUsername(userDetails);//generateTokenFromUsername(userDetails) extracts the username from UserDetails.
